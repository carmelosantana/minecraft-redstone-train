...
- [x] Fresh-volume [Legendary Java Minecraft Geyser Floodgate stack](https://github.com/TheRemote/Legendary-Java-Minecraft-Geyser-Floodgate) test covers every updater-managed plugin. → **7b (2026-08-16): MATRIX PASSED 19/19.** Full roster installed from published releases and enabled together on a fresh volume; 22 plugins green incl. Geyser-Spigot/floodgate/ViaVersion; Ollama & Umami disabled-by-default negative path held (server stayed up); only benign log line was MagicCarpet's optional WorldGuard soft-probe. (Run required the toolkit rig fix to pass a `GITHUB_TOKEN` to the updater overlay — an unauthenticated first attempt hit GitHub's 60/hr anonymous API cap and reported all 19 as false ABSENTs.)
- [x] Each updater-managed plugin's manifest `enabled` value, default state, and expected fresh-volume behavior are recorded separately. → **7b:** Redstone Train manifest `enabled` absent (=true), default-installs, expected present+enabled → observed `RedstoneTrain PRESENT` and `Enabling RedstoneTrain v0.1.0`. Full 19-row table in the ecosystem handbook's matrix-run record.
